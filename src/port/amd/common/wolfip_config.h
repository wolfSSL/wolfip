 ...
 *
 * Shared wolfIP profile for the AMD/Xilinx bare-metal ports. The three
 * boards use an identical UDP-only-intent profile, so it lives here and each
 * board's config.h is a thin shim that includes it. A board can still
 * override any value by #defining it before the include (the #ifndef-guarded
 * entries) or by editing its own config.h.
 */
#ifndef AMD_WOLFIP_CONFIG_H
#define AMD_WOLFIP_CONFIG_H
 ...
 * compile time. */
#ifdef SPEED_TEST
/* TCP throughput build (port 9 sink/source). Bigger per-socket rx/tx
 * buffers raise the TCP window (the rx free space the board advertises and
 * the tx data it can keep in flight); the UDP socket count is trimmed since
 * only DHCP needs UDP here. Every socket embeds rxmem[RXBUF_SIZE] +
 * txmem[TXBUF_SIZE], so the totals are kept within the 256 KB OCM budget
 * (the linker gates any overflow). */
#define MAX_TCPSOCKETS          2       /* listen + one client (passive close,
                                         * so no board-side TIME_WAIT slot) */
#define MAX_UDPSOCKETS          2       /* DHCP only */
