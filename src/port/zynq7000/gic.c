 ...
 *
 * GIC-390 (ARM GICv2) minimal driver for Cortex-A9 on Zynq-7000.
 *
 * Functionally identical to src/port/zcu102/gic.c -- both are
 * GICv2-compatible distributor+CPU IF designs. Only the base
 * addresses (in board.h) differ. The CPU runs Secure by default on
 * Zynq-7000 (no EL3 vs Secure-EL1 distinction); the ZCU102 startup
 * code's SCR_EL3 routing fix is therefore not applicable, and the
 * IRQ exception path comes through the ARMv7 vector at
 * VBAR + 0x18 (see startup.S). The same "GIC INTID == TRM
 * interrupt ID" convention applies (no GIC_SPI offset shift).
 *
...
#define GIC_NR_LINES        192    /* GIC-400 in ZynqMP supports up to 192 SPIs */
...
void irq_dispatch(void)
{
    ...
    if (intid < GIC_NR_LINES && handlers[intid] != 0)
        handlers[intid]();
    /* Always EOI to allow next interrupt, even if no handler matched. */
    GICC_EOIR = iar;
}
