...
/* Max poll cycles to spend in TLS_CLIENT_STATE_CONNECTING before giving up on a
 * refused/unreachable server (~10s at a few-ms poll cadence). */
#ifndef TLS_CONNECT_MAX_POLLS
#define TLS_CONNECT_MAX_POLLS 2000
#endif
...
/* Release the WOLFSSL object and its wolfIP IO-descriptor slot (idempotent).
 * wolfSSL_CleanupIO_wolfIP must run before wolfSSL_free or the io_descs pool
 * leaks one slot per session. */
static void tls_client_teardown(void)
{
    if (client.ssl) {
                       ...
                       tls_client_response_cb response_cb, void *user_ctx)
{
    ...
    if (host == NULL || host[0] == '\0') {
        debug_print("TLS Client: NULL/empty host\n");
        return -1;
    }
        ...
        case TLS_CLIENT_STATE_CONNECTING:
            /* Check if TCP connection is established by calling connect again */
            {
                ...
                if (ret == -WOLFIP_EAGAIN) {
                    /* Still connecting. A refused/failed connect re-SYNs and
                     * keeps returning EAGAIN, so bound the attempt. */
                    client.connect_ready_count = 0;
                    if (++client.connect_wait_count > TLS_CONNECT_MAX_POLLS) {
                        debug_print("TLS Client: Failed! connect timeout\n");
                        tls_client_finish(TLS_CLIENT_STATE_ERROR);
                        return -1;
                    }
                    return 0;
                }
        ...
        case TLS_CLIENT_STATE_CONNECTED:
            ...
            if (ret > 0) {
                ...
                if (err == WOLFSSL_ERROR_ZERO_RETURN) {
                    ...
                    tls_client_finish(TLS_CLIENT_STATE_DONE);
...
void tls_client_close(void)
{
    ...
    tls_client_teardown();   /* frees the SSL object and its IO slot */
    if (client.fd >= 0 && client.stack) {
        wolfIP_sock_close(client.stack, client.fd);
        client.fd = -1;
    }
