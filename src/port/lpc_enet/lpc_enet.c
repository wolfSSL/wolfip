...
#define MACCR_JE        (1U << 20)  /* Jumbo Enable */
#define MACCR_BE        (1U << 21)  /* CRC stripping for type packets / back-off */
#define MACCR_WD        (1U << 27)  /* Watchdog Disable */
...
/* Retry PHY detect a few times - some PHYs (LAN8742A) need extra time
 * after RMII clock comes up before MDIO is reliable. */
static int32_t phy_detect_retry(void)
{
    uint32_t attempt;
    for (attempt = 0; attempt < 10U; attempt++) {
        int32_t a = phy_detect();
        if (a >= 0) return a;
        /* Crude delay - 1k MDIO reads is roughly tens of ms at MDC=2.5MHz */
        for (uint32_t i = 0; i < 1000U; i++) {
            (void)mdio_read(0U, PHY_ID1);
        }
    }
    return -1;
}
...
static void phy_init(void)
{
    ...
    if (phy_addr < 0) {
        ...
        if (phy_addr < 0) return;  /* leave phy_addr=-1 to flag scan failure */
    }
...
int lpc_enet_phy_addr(void)
{
    return (int)phy_addr;
}

uint16_t lpc_enet_phy_read(uint8_t reg)
{
    if (phy_addr < 0) return 0xFFFFU;
...
static void config_mac(const uint8_t mac[6])
{
    ...
    ETH_MACCR = MACCR_JE | MACCR_BE | MACCR_WD |
...
int lpc_enet_init(struct wolfIP_ll_dev *ll, const uint8_t *mac)
{
    ...
    if (phy_addr < 0) {
        /* PHY not detected on MDIO bus, but MAC is initialized.
         * Caller can check via lpc_enet_phy_addr(). PHY may still
         * auto-negotiate from power-on defaults and link will come up. */
        return 0x100; /* link unknown - leave addr=0 in low byte */
    }
