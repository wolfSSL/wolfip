 ...
 *
 * Cadence GEM driver for Versal GEM0 (on-board RJ45 on VMK180).
 *
 * - 32-bit DMA addressing (OCM low bank, well under 4 GB).
 * - IRQ-driven RX via GIC-600 INTID 88 (GIC_SPI 56) -> gem_isr;
 *   polled TX. Register set is identical to ZynqMP's GEM3 -- only
 *   the base address (board.h GEM0_BASE) and INTID (board.h
 *   IRQ_GEM0) differ. The SCR_EL3.IRQ routing convention from the
 *   ZCU102 port is preserved (see startup.S) defensively, even
 *   though Versal's GICv3 + BL31 handoff path may not require it.
...
{
    volatile uint32_t *gem3_ref = (volatile uint32_t *)CRL_APB_GEM0_REF_CTRL;
    uint32_t div1;
    uint32_t val;
...
static void gem3_hw_reset(void)
{
    ...
    volatile uint32_t *gem3ref = (volatile uint32_t *)CRL_APB_GEM0_REF_CTRL;
