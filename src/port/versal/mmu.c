...
static void mmu_build_tables(void)
{
    ...
    for (i = 0; i < 512; i++) {
        ...
        if ((addr + L2_BLOCK_SIZE) <= dma_lo || addr >= dma_hi) {
            ...
            /* Any 2 MB block that overlaps the DMA region is Normal-NC
             * so newlib's memcpy/memset (LDP/STP, unaligned tails) does
             * not alignment-fault when staging frames into tx_buf_pool. */
            L2_DDR[i] = BLOCK_NORMAL_NC(addr);
        }
#endif /* VERSAL_LAYOUT_DDR */
    }
    ...
    /* Entry 511 covers 0xFFE00000..0xFFFFFFFF. OCM is at 0xFFFC0000+,
     * within this 2 MB block. Map as Normal WB cacheable, executable. */
