...
/* Where dns_result_cb() delivers the answer. One slot serves every context:
 * only one resolution runs at a time. */
...
/* Decide whether the host is a well-formed dotted quad or a name for the
 * resolver. */
                        ...
                        uint64_t start, int timeout_ms, ip4 *out)
{
    ...
    for (;;) {
        ...
        if (rc == 0)
            break;
        /* Retry on busy and on EAGAIN; any other failure is final. */
...
/* Only -WOLFIP_EAGAIN is reported as would-block; everything else ends the
 * transfer. */
...
/* Confirm the descriptor still names our connection before driving it. */
static int peer_is_ours(struct wolfcert_io_ctx *c, int fd, ip4 ip, int port)
{
    struct wolfIP_sockaddr_in peer;
    socklen_t len = sizeof(peer);

    memset(&peer, 0, sizeof(peer));
    if (wolfIP_sock_getpeername(c->stack, fd, (struct wolfIP_sockaddr *)&peer,
                                &len) != 0)
        return 0;
    return ((ee32(peer.sin_addr.s_addr) == ip) &&
            (ee16(peer.sin_port) == (uint16_t)port));
}
                                   ...
                                   int timeout_ms, void **conn)
{
    ...
    rc = wolfIP_sock_connect(c->stack, fd, (struct wolfIP_sockaddr *)&addr,
                             sizeof(addr));
    ret = WOLFCERT_ERR_IO;
    while ((rc == 0) || (rc == -WOLFIP_EAGAIN)) {
        if (!peer_is_ours(c, fd, ip, port)) {
            owned = 0;
            ret = WOLFCERT_ERR_CONN_CLOSED;
            break;
        }
        if (wolfIP_sock_can_write(c->stack, fd) == 1) {
            ret = WOLFCERT_OK;
            break;
        }
        if (deadline_expired(c, start, timeout_ms,
    ...
    /* Close only a descriptor that still names our peer. */
    if (ret != WOLFCERT_OK) {
        if (owned)
...
/* Start the close and leave the FIN handshake to the stack. */
