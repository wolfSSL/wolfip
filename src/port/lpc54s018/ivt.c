...
/* SPIFI config block (512 bytes at flash offset 0).
 * All-zeros is safe for debugger-loaded images. For standalone boot,
 * populate with Winbond W25Q128JV flash config from MCUXpresso SDK. */
__attribute__((section(".spifi_config"), used))
const uint32_t spifi_config[4] = { 0, 0, 0, 0 };

/* Vector table (at flash offset 0x200).
 * Entry[7] is patched by fix_checksum.py (entries 0-7 must sum to 0).
 * LPC54S018 has 73 external interrupts (IRQ 0-72). */
...
const uint32_t vector_table[16 + 73] = {
    [0] = (uint32_t)&_estack,
    ...
    [16 ... 88] = (uint32_t)&default_handler
};
