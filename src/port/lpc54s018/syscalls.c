...
/* FIFOSTAT bits 12-19 = TXLVL (number of bytes in TX FIFO).
 * FIFO depth is 8. TX is possible when TXLVL < 8.
 * Note: bit 5 is TXEMPTY (not TXNOTFULL as on some other chips). */
#define USART0_FIFOSTAT_TXLVL_MASK  (0xFFU << 12)
#define USART0_FIFOSTAT_TXLVL_MAX   (8U << 12)
...
int _write(int file, const char *ptr, int len)
{
    ...
    (void)file;
    for (i = 0; i < len; i++) {
        while ((USART0_FIFOSTAT & USART0_FIFOSTAT_TXLVL_MASK) >= USART0_FIFOSTAT_TXLVL_MAX) { }
