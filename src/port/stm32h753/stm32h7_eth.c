/* stm32h7_eth.c
 *
 * STM32H753ZI Ethernet driver
 *
 * Copyright (C) 2024 wolfSSL Inc.
 *
 * This file is part of wolfIP TCP/IP stack.
 *
 * The STM32H7 Ethernet MAC is a Synopsys DesignWare MAC similar to H5,
 * but with different peripheral addresses and GPIO configuration.
 */
#include <stdint.h>
#include <string.h>
#include "config.h"
...
/* STM32H7 Ethernet MAC base address */
...
/* Place buffers in D2 SRAM for ETH DMA access (see linker script) */
...
static int eth_hw_reset(void)
{
     ...
     * Some STM32H7 revisions need time before MDIO is accessible. */
    eth_delay(400000); /* ~1ms at 400MHz SYSCLK */

    /* Pre-configure MDIO clock divider (CR=4 for HCLK 150-250MHz).
     * Must be set before any PHY register access. */
...
static void eth_config_mac(const uint8_t mac[6])
{
    ...
    maccr |= ETH_MACCR_DM | ETH_MACCR_FES;  /* Full duplex, 100 Mbps */
    ETH_MACCR = maccr;
     ...
     * STM32H7 MAC_RXQ_CTRL0 defaults to 0 (queue disabled!) which means
     * received frames never reach the MTL/DMA path. Must set RXQ0EN=10b. */
    ETH_MACRXQC0R = (2U << 0);  /* RXQ0EN = 10b (enabled for DCB/Generic) */
...
static void eth_init_desc(void)
{
    ...
    /* Set buffer addresses and OWN bit for RX */
    for (i = 0; i < TX_DESC_COUNT; i++) {
...
static void eth_config_dma(void)
{
    ...
    ETH_DMACCR = 0;  /* DSL = 0 (16-byte descriptors) */
...
static uint16_t eth_mdio_read(uint32_t phy, uint32_t reg)
{
    uint32_t cfg;
    eth_mdio_wait_ready();
    /* CR = 4 for HCLK/102 (HCLK 150-250MHz range, 200MHz → ~2MHz MDC) */
...
static void stm32h7_eth_generate_mac(uint8_t mac[6])
{
...
int stm32h7_eth_init(struct wolfIP_ll_dev *ll, const uint8_t *mac)
{
    ...
    if (mac == NULL) {
        stm32h7_eth_generate_mac(local_mac);
        mac = local_mac;
    }
