...
void wolfIP_register_callback(struct wolfIP *s, int sock_fd, void (*cb)(int sock_fd, uint16_t events, void *arg), void *arg)
{
    ...
    if (sock_fd & MARK_TCP_SOCKET) {
        ...
        t->callback = cb;
        t->callback_arg = arg;
    } else if (sock_fd & MARK_ICMP_SOCKET) {
        if ((sock_fd & (~MARK_ICMP_SOCKET)) >= MAX_ICMPSOCKETS)
            return;
        t = &s->icmpsockets[sock_fd & ~MARK_ICMP_SOCKET];
        t->callback = cb;
        t->callback_arg = arg;
    }
}
