...
START_TEST(test_raw_socket_send_hdrincl_respected)
{
    ...
    ck_assert_int_ge(sd, 0);
    ck_assert_int_eq(wolfIP_sock_setsockopt(&s, sd, WOLFIP_SOL_IP, WOLFIP_IP_HDRINCL,
                &one, sizeof(one)), 0);
    ...
    ck_assert_uint_eq(last_frame_sent_size, ETH_HEADER_LEN + IP_HEADER_LEN + sizeof(payload));
    {
...
START_TEST(test_packet_socket_recv_frame)
{
    ...
    wolfIP_recv_ex(&s, TEST_PRIMARY_IF, frame, (uint32_t)(ETH_HEADER_LEN + sizeof(payload)));
...
START_TEST(test_sock_opts_unknown_level)
{
    ...
    ck_assert_int_eq(wolfIP_sock_getsockopt(&s, udp_sd, 0xFFFF, 0, &value, &len), -WOLFIP_EINVAL);
}
END_TEST
...
START_TEST(test_sock_opts_sol_ip_unknown_optname)
{
    ...
    ck_assert_int_eq(wolfIP_sock_getsockopt(&s, udp_sd, WOLFIP_SOL_IP, 0xFFFF, &value, &len), -WOLFIP_EINVAL);
}
END_TEST
