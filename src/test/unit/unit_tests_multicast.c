...
/* A deferred query response owed across a tick-source rollback is re-armed at a deadline in the new tick domain. */
START_TEST(test_multicast_igmp_query_report_rearmed_after_tick_rollback)
{
    struct wolfIP s;
    int sd;
    struct wolfIP_ip_mreq mreq;
    ...
    /* Let the join repeat fire, take the tick close to the 32-bit wrap, then
     * fill the heap the query response needs. */
    wolfIP_poll(&s, 1001);
    wolfIP_poll(&s, 0xFFFFFF00U);
    for (i = 0; i < MAX_TIMERS; i++) {
        t.expires = s.last_tick + 1000000U + (uint64_t)i;
        filler[i] = (uint32_t)timers_binheap_insert(&s.timers, t);
    }
    ...
    wolfIP_recv_ex(&s, TEST_PRIMARY_IF, frame, sizeof(frame));
    ck_assert_uint_eq(s.mcast[0].tmr_report, NO_TIMER);
    ck_assert_uint_ne(s.mcast[0].report_at, 0);

    /* The tick source restarts below the old domain, and a slot frees. */
    timer_binheap_cancel(&s.timers, filler[0]);
    wolfIP_poll(&s, 1000);
    last_frame_sent_size = 0;
    wolfIP_poll(&s, 1100);
