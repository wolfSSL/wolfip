...
START_TEST(test_dhcp_parse_offer_option_overload)
{
    ...
    *opt++ = DHCP_OPTION_END;
     ...
     * options (terminated by END) -> file -> sname (RFC 2131
     * sec.4.4.1); the mask is in sname, the server id in file. */
    memset(&s, 0, sizeof(s));
    wolfIP_init(&s);
    mock_link_init(&s);
    wolfIP_ipconfig_set(&s, 0x0A000001U, 0xFFFFFF00U, 0);
    ...
    *opt++ = DHCP_OPTION_END;
    ...
    field_opt = (struct dhcp_option *)((uint8_t *)field_opt + 6);
    /* sname is now the last scanned region (RFC 2131 sec.4.4.1), so its
     * option list must carry the terminating end option. */
    field_opt->code = DHCP_OPTION_END;
    field_opt->len = 0;
    field_opt = (struct dhcp_option *)msg.file;
    field_opt->code = DHCP_OPTION_SERVER_ID;
    ...
    /* --- Scenario 4: overload = both, the same option code present in
     * both overloaded fields. RFC 2131 sec.4.4.1 interprets the file
     * field before the sname field, and the offer parser is last-wins,
     * so the sname value must be the one kept (the old sname-first scan
     * let the file value win). --- */
    memset(&s, 0, sizeof(s));
    wolfIP_init(&s);
    mock_link_init(&s);
    wolfIP_ipconfig_set(&s, 0x0A000001U, 0xFFFFFF00U, 0);
    ...
    *opt++ = DHCP_OPTION_END;
    ...
    field_opt->len = 4;
    field_opt->data[0] = 0xFF; field_opt->data[1] = 0xFF;
    field_opt->data[2] = 0xFF; field_opt->data[3] = 0xF0;
    field_opt = (struct dhcp_option *)((uint8_t *)field_opt + 6);
    field_opt->code = DHCP_OPTION_END;
    field_opt->len = 0;
    ...
    ck_assert_int_eq(ret, 0);
    ck_assert_uint_eq(s.dhcp_server_ip, 0x0A000064U);
    /* sname is scanned after file: its mask is the last one seen and
     * must be the one kept. */
    ck_assert_uint_eq(s.dhcp_offered_mask, 0xFFFFFFF0U);
}
END_TEST
