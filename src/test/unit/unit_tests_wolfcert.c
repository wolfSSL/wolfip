...
START_TEST(test_wolfcert_io_connect_ip_literal)
{
    ...
    /* ee16/ee32 are load-bearing: wolfIP_sock_connect reads the address back
     * with ee32(), so a dropped or doubled swap sends us to a wrong host. */
...
START_TEST(test_wolfcert_io_connect_retries_eagain)
{
    WolfCertTransport t;
    void *conn = NULL;
    void *c;
    ...
    wc_connect_steps[1] = -WOLFIP_EAGAIN;
    wc_connect_steps[2] = 0;
    wc_connect_steps_len = 3;

    ck_assert_int_eq(t.connect(t.ctx, "10.0.0.1", 443, 5000, &conn),
                     WOLFCERT_OK);
    /* Each retry must drive the stack, or the handshake never completes. */
 ...
 * -WOLFIP_EAGAIN first. Nothing calls disconnect twice. */
START_TEST(test_wolfcert_io_disconnect_completes_async_close)
{
    WolfCertTransport t;
    void *c;
    ...
    wc_close_eagain_left = 3;

    ck_assert_int_eq(t.disconnect(t.ctx, (void *)(intptr_t)0x100),
                     WOLFCERT_OK);
    ck_assert_int_eq(wc_close_calls, 4);
    ck_assert_int_gt(wc_poll_calls, 0);
...
/* A FIN handshake that never completes must still return, or a stalled peer
 * hangs the caller. */
START_TEST(test_wolfcert_io_disconnect_deadline)
{
    WolfCertTransport t;
    void *c;
    ...
    wc_close_eagain_left = 1000000;
    wc_now_step_ms = WOLFCERT_WOLFIP_IO_TIMEOUT_MS;

    ck_assert_int_eq(t.disconnect(t.ctx, (void *)(intptr_t)0x100),
                     WOLFCERT_ERR_IO);
    ck_assert_int_lt(wc_close_calls, 5);

    wolfCert_Cleanup_wolfIP(c);
}
END_TEST

START_TEST(test_wolfcert_io_handle_survives_round_trip)
{
    WolfCertTransport t;
    void *conn = NULL;
    void *c;
    ...
    /* wolfIP marks every TCP descriptor, so the handle is opaque to wolfCert
     * and must come back to wolfIP_sock_close() unchanged. */
    wc_socket_ret = 0x100 | 3;
    ck_assert_int_eq(t.connect(t.ctx, "10.0.0.1", 8080, 0, &conn),
                     WOLFCERT_OK);
    ck_assert_int_eq((int)(intptr_t)conn, 0x103);
