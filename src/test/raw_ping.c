...
int main(int argc, char *argv[])
{
    ...
    for (seq = 1; seq <= NUM_PINGS; ++seq) {
        ...
        icmp->type = ICMP_ECHO;
        icmp->code = 0;
        icmp->un.echo.id = htons((uint16_t)pid);
        ...
        icmp_hdr = (struct icmphdr *)(recvbuf + (ip_hdr->ihl * 4));

        if (icmp_hdr->type == ICMP_ECHOREPLY &&
            ntohs(icmp_hdr->un.echo.id) == (uint16_t)pid) {

            gettimeofday(&recv_time, NULL);
            {
                struct timeval *sent_time_ptr = (struct timeval *)(icmp_hdr + 1);
                double sent_ms = sent_time_ptr->tv_sec * 1000.0 +
                                 sent_time_ptr->tv_usec / 1000.0;
    ...
    if (replies == 0) {
        fprintf(stderr, "No replies received.\n");
        return 1;
    }
